...
describe("PIM page - Define a report", () => {
  beforeEach(() => {
    ...
    cy.get("@location")
      .then((location) => {
        AdminPage.createLocationViaAPI(location).then((response: any) => {
  ...
  it("User create report and view data", () => {
    ...
    cy.get("@reportDef").then((reportDefine) => {
      ...
      PIMpage.open();
      PIMpage.viewReports();
      PIMpage.defineReportViaUI(reportDefine).then((response: any) => {
        reportId = response.request.query.reportId;
      });
    });
    ...
    cy.get("@reportInfo").then((reportInfo: any) => {
      ...
      for (let index: number = 0; index < repoHeaders.length - 1; index++) {
        if (index == 1) {
          PIMpage.assertReport(repoHeaders[index], jobTitleName);
        }
        PIMpage.assertReport(repoHeaders[index], repoValues[index]);
      }
    });
  });
