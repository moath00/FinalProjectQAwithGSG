...
export default class PIM {
  elements = {
    ...
    reportLocators: {
      reportHeader: () => cy.get("revogr-header"),
      reportBody: () => cy.get('[type="rgRow"]'),
    },
  };
  ...
  defineReportViaUI(data: any) {
    ...
    cy.wait("@report");
    let reportId = cy.url().then((url) => url.split("/")[7]);
    reportId.then((id) => {
      cy.intercept(
        `https://opensource-demo.orangehrmlive.com/web/index.php/api/v2/pim/reports/data?limit=50&offset=0&reportId=${id}&name=pim_defined&_dateFormattingEnabled=true`
      ).as("reportContent");
      cy.wait("@reportContent", { timeout: 5000 });
    });
  ...
  assertReport(columnHeader: string, expectedValue: string) {
    ...
    this.elements.reportLocators
      .reportHeader()
      // .find(".rgHeaderCell")
      .contains(".rgHeaderCell", columnHeader)
      .invoke("index")
      .then((columnIndex: any) => {
        // I Find all rows in the table body
        this.elements.reportLocators.reportBody().each((elem) => {
          cy.wrap(elem)
            .find(".rgRow")
            .find(".rgCell")
            .eq(columnIndex)
